...
(function(){
  ...
  if (heroStage && heroCanvas && heroCanvas.getContext) {
    ...
    function resizeHeroField(){
      ...
      // These particles sit behind the instrument; full Retina resolution
      // adds four times the pixels without improving the visible result.
    ...
    function drawHeroField(now = 0){
      if (!reduced && now && now - heroLastFrame < 1000 / 60 - 1) {
        hraf = requestAnimationFrame(drawHeroField);
        return;
      }
      const step = heroLastFrame && now ? Math.min(2, (now - heroLastFrame) / 16.67) : 1;
      heroLastFrame = now;
    ...
    if (reduced) {
      ...
      const syncHero = () => {
        if (heroVisible && !document.hidden) {
          if (!hraf) hraf = requestAnimationFrame(drawHeroField);
  ...
  if (scatterCanvas && signalLab && scatterCanvas.getContext) {
    ...
    // Match the 64-signal label shown in the diagram; the extra off-copy
    ...
    function drawSignalLab(now=0){
      ...
      const step=signalLastFrame&&now?Math.min(2,(now-signalLastFrame)/16.67):1;
      signalLastFrame=now;
      ...
      dots.forEach((d,i)=>{
        ...
        // A few highlighted signals are enough to suggest glow. Blurring
        // every downstream dot was the expensive part of this canvas.
        ctx.shadowBlur=d.accent?7:0; ctx.shadowColor='rgba(124,108,240,.7)';
    ...
    if(reduced){drawSignalLab();}
    else{
      const syncSignal=()=>{
        if(signalVisible&&!document.hidden){
          if(!raf)raf=requestAnimationFrame(drawSignalLab);
        }else{
      ...
      if('IntersectionObserver' in window){
        const io=new IntersectionObserver(entries=>{
          signalVisible=entries.some(e=>e.isIntersecting);
          syncSignal();
        },{threshold:.08});
        io.observe(signalLab);
      }else{signalVisible=true;syncSignal();}
      document.addEventListener('visibilitychange',syncSignal);
    }
  }
